...
class Solution {
    ...
    public int[] topKFrequent(int[] nums, int k) {
        HashMap<Integer, Integer> mappedFrequencies = new HashMap<>();
        for (int i = 0; i < nums.length; i++) {
            mappedFrequencies.put(nums[i], mappedFrequencies.getOrDefault(nums[i], 0) + 1);
        }

        PriorityQueue<Integer> minHeap = new PriorityQueue<>((a, b) -> mappedFrequencies.get(a) - mappedFrequencies.get(b));
        
        for (int key : mappedFrequencies.keySet()) {
            minHeap.add(key);
            if (minHeap.size() > k) {
                minHeap.poll();
            }
